...
- 设置区：主题、语言、开机自启、全局 `no_proxy`、Shell 集成状态、移除集成。
- 导入区：当扫描到既有代理时显示导入候选，用户确认后才写入配置。

卡片只用于代理条目、导入候选、设置分组。禁止卡片套卡片。列表和设置面板优先使用边框、分隔线和留白建立层级。
...
动效只用于状态变化和反馈，例如启用开关、列表项出现、设置保存、导入确认。常规转场 150-250ms，使用 ease-out 曲线。
